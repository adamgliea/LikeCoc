...
{
	// must be done before [super init] to ensure default model/controllers are added before initWithDefaults runs
	if (sceneModel == nil)
	{
		KTSceneModel* defaultModel = [KTSceneModel model];
		[defaultModel load];
		[self internal_setModel:defaultModel];
	}
...
-(void) addSceneModel:(KTSceneModel*)sceneModel
{
	if (sceneModel == nil && self.createModelBlock)
	{
		KTModel* model = self.createModelBlock();
		if ([model isKindOfClass:[KTSceneModel class]])
		{
			sceneModel = (KTSceneModel*)model;
		}
		else
		{
			NSAssert(nil, @"KTSceneViewController's createModelBlock did not return a model of class KTSceneModel. Subclass your model from KTsceneModel instead of any other model class.");
		}
	}
