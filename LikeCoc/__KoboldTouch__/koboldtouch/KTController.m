...
-(void) internal_addSubController:(KTController*)controller insertAsFirstController:(BOOL)insertAsFirstController
{
	...
	if (_subControllers)
	{
		...
		NSAssert([controller isKindOfClass:[KTSceneViewController class]] == NO ||
				 ([controller isKindOfClass:[KTSceneViewController class]] && [self isKindOfClass:[KTGameController class]]),
				 @"KTSceneViewController objects can only be presented, not added as subController to other view controllers");
